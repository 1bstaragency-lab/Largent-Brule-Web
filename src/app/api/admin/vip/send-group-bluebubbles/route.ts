// POST /api/admin/vip/send-group-bluebubbles { groupId: number }
// Sends VIP early access via BlueBubbles instead of Blooio
import { isAdmin } from '@/lib/admin-auth';
import { supabaseAdmin, normalizePhone } from '@/lib/supabase-admin';
import { bluebubblesSend } from '@/lib/bluebubbles';
...
export async function POST(req: Request) {
  ...
  for (const member of groupMembers) {
    ...
    const idempotencyKey = `${e164}_vip_group_${groupId}_bb_v1`;
    const result = await bluebubblesSend({
      address: e164,
      text: messageBody,
      tempGuid: idempotencyKey,
    });
