  ...
  if (!isAuthenticated && !showPassword && !videoWatched) {
    return (
      <div className="w-full min-h-screen bg-black flex items-center justify-center lg:bg-white lg:px-4">
        {/* Mobile Video Entry */}
        <div className="w-screen h-screen lg:hidden flex flex-col items-center justify-center bg-amber-50 relative fixed inset-0">
          <Link href="/" className="absolute top-6 left-1/2 transform -translate-x-1/2 w-40 h-14 z-10 hover:opacity-70 transition-opacity">
            <Image
              src="/lb vip logo.png"
              alt="Home"
              fill
          ...
          <div className="w-full h-full border-4 border-stone-300">
            <video
              autoPlay
              muted
              loop
          ...
          <button
            ...
            className="absolute bottom-24 left-1/2 transform -translate-x-1/2 text-white text-2xl font-light tracking-wide bg-white/20 backdrop-blur-sm px-8 py-2 hover:bg-white/30 transition-all z-10"
